...
def test_mobilenet():
    ...
    train_ds, test_ds, class_names = data_load("",
                                              "./Spatial/slice_png/29db", 224, 224, 16)
    # todo 加载模型，修改为你的模型名称
...
def test_cnn():
    # todo 加载数据, 修改为你自己的数据集的路径
    train_ds, test_ds, class_names = data_load("./Spatial/slice_png/29db",
                                        "./Spatial/slice_png/29db", 224, 224, 32)
    # todo 加载模型，修改为你的模型名称
    model = tf.keras.models.load_model("./models/early_png.h5")
    # model.summary()
    # 测试
    loss, accuracy = model.evaluate(test_ds)
    # 输出结果
    ...
    # title, x_labels, y_labels, harvest
    show_heatmaps(title="heatmap", x_labels=class_names, y_labels=class_names, harvest=heat_maps_float,
                  save_name="./Spatial/results/0db.png")
    with open('./Spatial/slice_png_test.txt', 'a+') as f:
        line = '%s\n' % (accuracy)
        f.write(line)
